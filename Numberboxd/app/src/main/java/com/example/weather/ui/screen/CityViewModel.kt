...
class CityViewModel: ViewModel() {
    ...
    fun getID(cityItem: cityItem): String{
        return cityItem.ID
    }

    fun updateShowRating(cityItem: cityItem, newRating: String) {
        val index = _cityList.indexOfFirst { it == cityItem }
        if (index != -1) {
            val updatedCityItem = cityItem.copy(showRating = newRating)
            _cityList[index] = updatedCityItem
        }
    }
